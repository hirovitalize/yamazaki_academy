...
module HasDatetimeRange
  ...
  module ClassMethods
    ...
    def acts_as_datetimerange(
      ...
      validates to, timeliness: { on_or_after: from.to_sym, type: :datetime },
                    allow_blank: true,
                    if: proc { |model| model.send(from).present? }
      validates from, timeliness: { type: :datetime }, allow_blank: true
    end
  end
end
